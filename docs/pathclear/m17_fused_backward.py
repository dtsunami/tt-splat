...
# M17 — FUSED BACKWARD SFPU KERNEL (Phase 2b keystone).
# STATUS: single-Gaussian (S=0) all-7-grads VERIFIED on silicon vs torch (err/scale 5.8e-3, bf16 dst).
# Proves the hard part: reverse-blend arithmetic + recip-reconstruct-T + copy_dest_values fused in ONE
# generic_op compute kernel. REMAINING (mechanical): (1) S/T_run reverse-loop state for K>1 via CB
# recurrence; (2) multi-tile (M14 sharded grid); (3) M2 scatter-add cross-tile; (4) wire behind
# DeviceRaster.backward. GOTCHAS: bf16 dst=16 slots (uses d0-15); fp32_dest_acc HALVES to 8 -> aliases
# d8-15 (do NOT enable as-is); needs copy_dest_values.h + tile_move_copy.h includes; scalars = f2u bits.
